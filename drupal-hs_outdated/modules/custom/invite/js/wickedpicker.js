...
(function ($, window, document) {
    ...
    $.extend(Wickedpicker.prototype, {
        ...
        createPicker: function () {
            if ($('.wickedpicker').length === 0) {
                ...
                picker += '<li class="wickedpicker__controls__control"><span class="' + this.options.upArrow + '"></span><span class="wickedpicker__controls__control--meridiem" tabindex="-1">AM</span><span class="' + this.options.downArrow + '"></span></li></ul><p class="wickedpicker__timeperiod"><input type="radio" name="timeperiod" id="15min" value="15min" checked="checked"><label for="15min">15 min</label><input type="radio" name="timeperiod" id="30min" value="30min"><label for="30min">30 min</label><input type="radio" name="timeperiod" id="45min" value="45min"><label for="45min">45 min</label></p></div>';
                $('body').append(picker);
                custom_timepicker();
  ...
  function custom_timepicker(){
    var time = jQuery("#edit-starttime").val();
    var radioValue = parseInt(jQuery("input[name='timeperiod']:checked").val());
    var splitted_time = time.split(':');
    if(splitted_time[1]){
      var trimed_start_time = splitted_time[0].trim().concat(':'+splitted_time[1].trim());
    }
    jQuery('#edit-starttime').val(trimed_start_time);
    var start_time = jQuery('#edit-starttime').val();
    var d = new Date("October 13, 2016 "+start_time);
    d.setMinutes(d.getMinutes()+radioValue);
    //console.log("minute--"+d.getMinutes()+'--');
    var incr_minute = d.getMinutes();
    if(d.getMinutes() === 0){
      incr_minute = '00';
    }
    if(d.getMinutes() > 0 && d.getMinutes() <= 9){
      incr_minute = '0'+d.getMinutes();
    }
    var created_end_time = d.getHours()+":"+incr_minute;
   // var created_end_time = tconvert(end_time);
    jQuery('#edit-endtime').val(created_end_time);
    jQuery("#edit-duration").val(radioValue);
  }
